...
class HyperIQASolver(object):
    ...
    def train(self):
        ...
        for t in range(self.epochs):
            ...
            for img, label in tqdm(self.train_data, total=len(self.train_data)):
            ...
            if test_srcc > best_srcc:
                best_srcc = test_srcc
                best_plcc = test_plcc
                folder_res = "pretrained/AGIQA_3k"
                torch.save(self.model_hyper.state_dict(), f"{folder_res}/{t + 1}_AGIQA_3k.pt")

            with open('metrics.csv', 'a', newline='') as csvfile:
                    fieldnames = ['Epoch', 'Train_Loss', 'Test_SRCC', 'Test_PLCC']
